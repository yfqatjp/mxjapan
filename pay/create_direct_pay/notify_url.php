    ...
    if ($trade_status == 'TRADE_FINISHED' || $trade_status == 'TRADE_SUCCESS') {
        ...
        if ($rs->rowCount() > 0) {
            ...
            if (isset($row['type']) && $row['type'] == 1) {
            	...
            	$otherData["payment_total"] = $row["price"];
            	$arrBookingData = $hmWeb->getBookingData($row["id"], $row, $row5, $row1, 1, 4, $otherData);
            	//
            	$charterBookingSql = "select * from pm_charter_booking where trans = ? ";
            	$arrExistsBooking = $hmWeb->findOne($charterBookingSql, array($out_trade_no));
