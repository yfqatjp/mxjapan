...
if(isset($_POST['uniqid']) && isset($_POST['timestamp']) && isset($_POST['dir']) && isset($_POST['root_bo']) && isset($_POST['fileType']) && isset($_POST['exts'])){
    ...
    if(!empty($_FILES) && $_POST['token'] == $verifyToken){
        ...
        $uniqid = $_POST['fileType'];
        
        // upload folder for a session
        if(!is_dir($path."/".$verifyToken)) mkdir($path."/".$verifyToken, 0777);
        chmod($path."/".$verifyToken, 0777);
        if(!is_dir($path."/".$verifyToken."/".$userId)) mkdir($path."/".$verifyToken."/".$userId, 0777);
        chmod($path."/".$verifyToken."/".$userId, 0777);
        if(!is_dir($path."/".$verifyToken."/".$userId."/".$uniqid)) mkdir($path."/".$verifyToken."/".$userId."/".$uniqid, 0777);
        chmod($path."/".$verifyToken."/".$userId."/".$uniqid, 0777);
        ...
        $targetFile = $path."/".$verifyToken."/".$userId."/".$uniqid."/".$filename;
