...
$rs = $pdo->query("SELECT *,a.room AS aroom FROM pm_gwc AS a LEFT JOIN pm_booking AS b ON a.onum = b.trans WHERE b.id IS NOT NULL AND (b.`status` = 4 OR b.`status` = 5) AND b.`id_room` = ".$_GET['a']);
while ($row = $rs->fetch()) {
    if ($dis != "") {
        $dis .= ",";
    }
