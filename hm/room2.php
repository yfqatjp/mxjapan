...
if (@$_GET['d'] != "" && @$_GET['e'] != "") {
    function getDateFromRange($startdate, $enddate)
    {
        ...
        $date = "'" . date('Y-m-d', $stimestamp + (86400 * 0)) . "'";
        for ($i = 1; $i < $days; $i++) {
...
$rs = $pdo->query("SELECT *,a.room AS aroom FROM pm_gwc AS a LEFT JOIN pm_booking AS b ON a.onum = b.trans WHERE b.id IS NOT NULL AND (a.tai = 1 OR a.tai = 3) ");
while ($row = $rs->fetch()) {
    if ($dis!="") {
        $dis .= ",";
    }
    ...
    <div class="midd_59"><span>入住日期：</span>
        <div class="midd_60">
            <input name="ont" class="rendezvous-input-date"
                   onfocus="var date=randomDate2();WdatePicker({maxDate:'#F{$dp.$D(\'end\')}',minDate:date<?php echo $dis; ?>})"
    ...
    <div class="midd_59"><span>退房日期：</span>
        <div class="midd_60">
            <input name="offt" class="rendezvous-input-date"
                   onfocus="var date=randomDate();WdatePicker({minDate:'#F{$dp.$D(\'start\')}',minDate:'<?php echo date('Y-m-d');?>',maxDate:date<?php echo $dis; ?>})"
                   data-validation-engine="validate[required]"
                   id="end">
        </div>
        <div class="clear"></div>
...
<script type="text/javascript">
    function randomDate() {
        b = '<?php echo date('Y-m-d', $_GET['d'] + 365 * 60 * 60 * 24);?>';
        a = $('#start').val();
        var timestamp2 = Date.parse(new Date(a));
        timestamp2 = timestamp2 / 1000;
        if (a != "") {
            if (timestamp2 <<?php echo $_GET['d'];?>) {
                b = '<?php echo date('Y-m-d', $_GET['d']);?>';
            }
        }
        return b;
    }
    ...
    function randomDate2() {
        ...
        timestamp2 = timestamp2 / 1000;
        if (a != "") {
            if (timestamp2 >=<?php echo $_GET['e'];?>) {
                b = '<?php echo date('Y-m-d', $_GET['e']);?>';
            }
        }
        return b;
    }
