...
$perNumber = 6;
$page = @$_GET['page'];
$count = $pdo->query("SELECT * FROM pm_realestate WHERE lang = 2 AND checked = 1");
...
$rs = $pdo->query("SELECT * FROM pm_realestate where lang = 2 AND checked = 1 limit $startCount,$perNumber");
while ($row = $rs->fetch()) {
    ?>
    <div class="midd_auto midd_fff midd_top20 midd_91"><a href="realestatexx_x<?php echo $row['id'] ?>.html">
            <div class="image"><img
                    src="<?php $rs1 = $pdo->query("SELECT * FROM pm_realestate_file WHERE id_item = " . $row['id'] . " ");
                    $row1 = $rs1->fetch();
